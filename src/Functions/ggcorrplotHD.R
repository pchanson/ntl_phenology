ggcorrplotHD <- function(corr, method = c("square", "circle"), type = c("full", 
          ...
          tl.col = "black", tl.srt = 45, digits = 2, as.is = FALSE) 
{
  ...
  if (inherits(corr, "cor_mat")) {
    cor.mat <- corr
    corr <- ggcorrplot:::.tibble_to_matrix(cor.mat)
    p.mat <- ggcorrplot:::.tibble_to_matrix(attr(cor.mat, "pvalue"))
  }
  ...
  if (hc.order) {
    ord <- ggcorrplot:::.hc_cormat_order(corr, hc.method = hc.method)
    corr <- corr[ord, ord]
    if (!is.null(p.mat)) {
      p.mat <- p.mat[ord, ord]
  ...
  if (!show.diag) {
    corr <- ggcorrplot:::.remove_diag(corr)
    p.mat <- ggcorrplot:::.remove_diag(p.mat)
  }
  if (type == "lower") {
  ...
  corr <- reshape2::melt(corr, na.rm = FALSE, as.is = as.is)
  colnames(corr) <- c("Var1", "Var2", "value")
  corr$pvalue <- rep(NA, nrow(corr))
  corr$signif <- rep(NA, nrow(corr))
  if (!is.null(p.mat)) {
    p.mat <- reshape2::melt(p.mat, na.rm = FALSE)
  ...
  p <- ggplot2::ggplot(data = corr, mapping = aes(x = Var1, y = Var2, fill = value))
