...
library(tidyverse)
dat = read_csv("../Data/final_metric_data/final_combined_dates.csv")
...
vars = c("iceoff", "straton", "secchi_all", "secchi_openwater", "daphnia_biomass",
                               "total_zoop_biomass", "chlor_all", "chlor_spring", "chlor_fall", "doc", 
...
cor = n_lakes_wide %>% 
  group_by(lakeid) %>% 
  summarise(r = paste("r =", round(cor(doc, doc_predict, use="complete.obs"), 3)))

n_lakes_wide %>% 
  ggplot(aes(x=doc, y=doc_predict, color=as.character(lakeid))) +
  geom_point()+
  theme_bw() +
  labs(color="lakeid")+
...
missing_doc = n_lakes_wide %>% filter(is.na(doc))
...
```{r}
filled_chl_dates = read_csv("../Data/chl_modeled_peaks.csv") %>% 
...
write_csv(dat_final, "../Data/final_combined_dates_filled_v1.csv")
```
