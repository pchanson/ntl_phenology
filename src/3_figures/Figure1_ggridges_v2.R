...
vars_label = c("","ice off", "strat onset", "stability", "energy", 'strat offset','ice on',
               'DOC epi max', 'Si min', 'TN epi min', 'TP epi min', 
...
plotridge <- function(uselakes) {
  dat %>% 
    ...
    scale_fill_manual(values = rev(c(rep('#e3d35d',6), rep('#97bab7',6), rep('#bf7058',4)))) +
    scale_color_manual(values = rev(c(rep('#e3d35d',6), rep('#97bab7',6), rep('#bf7058',4)))) +
...
plotiqr <- function(uselakes) {
  ...
  ggplot() + 
    ...
    xlab('IQR (days)') +
    scale_fill_manual(values = rev(c(rep('#e3d35d',7), rep('#97bab7',6), rep('#bf7058',4)))) +
    # scale_fill_manual(values=met.brewer("Archambault", length(vars_order))) + 
    theme_minimal(base_size = 8) +
    # labs(title = 'IQR (days)') +
    xlim(0,200) +
...
ggsave('Figures_manuscript/Figure1_v2.pdf',
       width=6, height=8, units="in", dpi=500, bg='transparent')
